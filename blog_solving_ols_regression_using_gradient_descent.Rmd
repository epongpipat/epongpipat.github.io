...
We can see from these random samples of intercepts and slopes that the lowest cost is `r round(min(df_gd$cost), 2)` with an intercept of `r subset(df_gd, cost == min(cost))$int` and slope of `r subset(df_gd, cost == min(cost))$slope`. So no matter where we start (any intercept and slope combination), we should descend and ultimately end up with the lowest cost value of `r round(min(df_gd$cost), 2)`.
...
### Cost
```{r}
glue("Cost using GD: {df_min$cost}\nCost using lm(): {glance(model)$sigma^2}")
```
